...
\textit{\textbf{Instructions:} This worksheet presents two types of hypothesis
  tests involving the chi-squared statistic. Please work in groups. You will
  be asked to turn in your group worksheet on Wednesday, May 6.}



\phantom{don't write in starlight, cause the words may come out real}
\begin{center}
  {\small\begin{tcolorbox}[ width=1\textwidth, colback=white, colframe=black,
      title=Background on chi-squared statistic, colbacktitle=blue!15,
      coltitle=black ] Suppose we have an experiment with $k$ possible
      outcomes, labelled $1,\ldots,k$, such that outcome $i$ occurs with
      probability $p_{i}$. We replicate the experiment $n$ times, keeping
      track of the number of times each outcome has occurred. In this setting,
      the \demph{chi-squared statistic} is defined as
    \begin{equation*}
      \chi^{2}:= \sum_{i=1}^{k} \frac{\left(N_{i}-E_{i}\right)^{2}}{E_{i}},
    \end{equation*}
    where
    \begin{align*}
      N_{i}&= (\text{\# of times outcome $i$ occurs})\\
      E_{i} &= (\text{expected \# of times outcome $i$ occurs})
    \end{align*}
    We'll illustrate the use of this statistic in two different types of
    hypothesis tests:
    \begin{itemize}
      \item For a \textbf{goodness of fit test}, $\chi^{2}$ has a chi-squared
      distribution with $k-1$ degrees of freedom.
      \item For a \textbf{test of independence} using an $r\times c$
      contingency table, $\chi^{2}$ has a chi-squared distribution with
    ...
    \end{equation*}
    where $\Gamma(k)=(k-1)!$ for any positive integer $k$.
  \end{tcolorbox}}
\end{center}
...
\begin{problem}[Goodness-of-fit test]
  ...
  \begin{enumerate}[(a)]
    ...
    $\P\left[\chi^{2}\geq u\right]$, where $u$ is the value you got in part
    \ref{item:4}.
    \vspace{8cm}
...
\newpage
\subsection{Hypothesis test type \#2: Test of independence}
...
\begin{problem}[Another example of Simpson's Paradox]
  ...
  defendant's race, the victim's race, and whether or not the defendant was
  sentenced to death.
  \vspace{.5em}
  \begin{center}
    \begin{tabular}{cc}
  ...
  \begin{enumerate}[(a)]
    ...
    things?
    \vspace{1cm}
    \item Suppose you were to perform a chi-squared test of independence for
    the table you just made. You get $p=.64$. How would you interpret this
    result?
    ...
    \end{itemize}
    \item Basing your reasoning on the data, explain why the paradox holds in
    language that a judge who doesn't know any statistics could understand.
    Since you are addressing a judge, your answer should begin with the words,
    ``Your Honor...''
  \end{enumerate}
